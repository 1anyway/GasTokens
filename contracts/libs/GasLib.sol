...
library GasLib {
    ...
    function mulDiv(
        ...
        (uint256 l, uint256 h) = FullMath.fullMul(x, y);
    ...
    function muli(
        ...
        uint256 z = mulDiv(self._x, uint256(y < 0 ? -y : y), Q112);
        require(z < 2 ** 255, "FixedPoint::muli: overflow");
        return y < 0 ? -int256(z) : int256(z);
    }

    // multiply a UQ112x112 by an uint and decode, returning an uint
    // reverts on overflow
    function mulu(
        uq112x112 memory self,
        uint256 y
    ) internal pure returns (uint256) {
        uint256 z = mulDiv(self._x, y, Q112);
        require(z < 2 ** 255, "FixedPoint::muli: overflow");
        return z;
    }

    // multiply a UQ112x112 by a UQ112x112, returning a UQ112x112 lossy
    function muluq(
        uq112x112 memory self,
        uq112x112 memory other
    ) internal pure returns (uq112x112 memory) {
        if (self._x == 0 || other._x == 0) return uq112x112(0);
    ...
    function fraction(
        ...
        if (numerator == 0) return GasLib.uq112x112(0);
